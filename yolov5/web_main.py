...
from utils.general import non_max_suppression, scale_coords, check_imshow #letterbox,
    ...
    for path, img, img0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):
            ...
            if not os.path.exists(log_dir):
                os.makedirs(log_dir)
            log_dir='log//'+datetime.datetime.now().strftime('%Y%m%d')+'//log.txt'
            with open(log_dir,'a+') as log_data:
                log_time=datetime.datetime.now().strftime('%Y%m%d %H:%M:%S')
                log_data.write(f'{log_time} {s}Done. ({t2 - t1:.3f}s)'+'\n')
            #####
